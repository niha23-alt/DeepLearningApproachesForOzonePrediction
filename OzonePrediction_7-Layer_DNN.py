...
OPTIMIZER = "adam"
BATCH_SIZE = 32
model_name = '7-layer-DNN-new-106'

def build_7Layer_DNN(dnn_size, LOSS_FUNCTION, OPTIMIZER, ACT_FUN, use_dropout):
    inputs = Input(shape=(dnn_size,))

    batch_nor = BatchNormalization()(inputs)
    dense_0 = Dense(dnn_size, activation=ACT_FUN)(batch_nor)

    dense_0 = Dense(512, activation=ACT_FUN)(dense_0)

    batch_nor_0 = BatchNormalization()(dense_0)

    if use_dropout:
        dropout_layer_2 = Dropout(0.3)(batch_nor_0)
        dense_1 = Dense(256, activation=ACT_FUN)(dropout_layer_2)
    else:
        dense_1 = Dense(256, activation=ACT_FUN)(batch_nor_0)

    batch_nor_1 = BatchNormalization()(dense_1)

    if use_dropout:
        dropout_layer_3 = Dropout(0.3)(batch_nor_1)
        dense_2 = Dense(128, activation=ACT_FUN)(dropout_layer_3)
    else:
        dense_2 = Dense(128, activation=ACT_FUN)(batch_nor_1)

    batch_nor_2 = BatchNormalization()(dense_2)

    if use_dropout:
        dropout_layer_4 = Dropout(0.3)(batch_nor_2)
        dense_3 = Dense(64, activation=ACT_FUN)(dropout_layer_4)
    else:
        dense_3 = Dense(64, activation=ACT_FUN)(batch_nor_2)

    batch_nor_4 = BatchNormalization()(dense_3)

    dense_4 = Dense(32, activation=ACT_FUN)(batch_nor_4)

    dense_5 = Dense(1, activation='sigmoid')(dense_4)

    model = Model(inputs=inputs, outputs=dense_5, name=model_name)
...
Neural_model = build_7Layer_DNN(106, LOSS_FUNCTION, OPTIMIZER, 'relu', True)
Neural_model.summary()

callbacks_list_new = [
    ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=10, mode='auto', min_lr=0, verbose=1),
...
predicted_data_normalized = normalized_test_set # This is the normalized data.
predicted_data_normalized[:,0] = probs.flatten() 

predicted_data = scaler.inverse_transform(predicted_data_normalized) # Perform inverse normalization.

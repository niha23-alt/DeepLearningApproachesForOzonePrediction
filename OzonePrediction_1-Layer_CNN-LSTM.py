...
OPTIMIZER = "adam"
BATCH_SIZE = 32
model_name = '1-layer-CNN-LSTM'

def build_CNNLSTM(dnn_size, LOSS_FUNCTION, OPTIMIZER, ACT_FUN, use_dropout):
    inputs = Input(shape=(106,))
    reshape_0 = Reshape((1, 106, 1))(inputs)
    batch_nor = BatchNormalization()(reshape_0)
    # define CNN model
    time_dis_1 = TimeDistributed(Conv1D(8, kernel_size=3, padding='same'))(batch_nor)
    time_dis_2 = TimeDistributed(MaxPooling1D())(time_dis_1)
    time_dis_flatten = (TimeDistributed(Flatten()))(time_dis_2)
    # define LSTM model
    lstm_0 = CuDNNLSTM(64, return_sequences=True)(time_dis_flatten)
    # lstm_1 = CuDNNLSTM(64, return_sequences=True)(lstm_0)
    flatten_0 = Flatten()(lstm_0)
...
Neural_model = build_CNNLSTM(106, LOSS_FUNCTION, OPTIMIZER, 'relu', True)

...
OPTIMIZER = "adam"
BATCH_SIZE = 32
model_name = '2-conv1d-CNN-new'

def build_CNN(dnn_size, LOSS_FUNCTION, OPTIMIZER, ACT_FUN, use_dropout):

    inputs = Input(shape=(106, ))
    reshape_0 = Reshape((106, 1))(inputs)
    batch_nor = BatchNormalization()(reshape_0)
    conv1d_1 = Conv1D(8, kernel_size=3, padding='same')(batch_nor) 
    batch_nor_1 = BatchNormalization()(conv1d_1)
    activ_1 = Activation(ACT_FUN)(batch_nor_1)
    conv1d_2 = Conv1D(8, kernel_size=4, padding='same')(activ_1)
    batch_nor_2 = BatchNormalization()(conv1d_2)
    activ_2 = Activation(ACT_FUN)(batch_nor_2)
    max_pool_2 = MaxPooling1D(padding='valid')(activ_2)
    flatten_0 = Flatten()(max_pool_2)
    batch_nor_1 = BatchNormalization()(flatten_0)
    dense_0 = Dense(dnn_size, activation=ACT_FUN)(batch_nor_1)
    if use_dropout:
        dropout_layer_2 = Dropout(0.5)(dense_0)
        dense_1 = Dense(int(dnn_size/2), activation=ACT_FUN)(dropout_layer_2)
    else:
        dense_1 = Dense(int(dnn_size/2), activation=ACT_FUN)(dense_0)
...
Neural_model = build_CNN(106, LOSS_FUNCTION, OPTIMIZER, 'relu', True)

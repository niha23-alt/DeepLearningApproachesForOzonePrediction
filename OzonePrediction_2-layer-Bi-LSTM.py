...
OPTIMIZER = "adam"
BATCH_SIZE = 32
model_name = '2-layer_Bi-LSTM_new-106'

def build_2LSTM(dnn_size, LOSS_FUNCTION, OPTIMIZER, ACT_FUN, use_dropout):
    inputs = Input(shape=(1,106))
    batch_nor = BatchNormalization()(inputs) 
    lstm_0 = Bidirectional(CuDNNLSTM(64, return_sequences=True))(batch_nor)
    lstm_1 = Bidirectional(CuDNNLSTM(64, return_sequences=True))(lstm_0)
    flatten_0 = Flatten()(lstm_1)
    batch_nor_1 = BatchNormalization()(flatten_0)
    dense_0 = Dense(192, activation=ACT_FUN)(batch_nor_1)
    batch_nor_2 = BatchNormalization()(dense_0)   
    if use_dropout:
        dropout_layer_2 = Dropout(0.2)(batch_nor_2)
        dense_1 = Dense(int(192 / 2), activation=ACT_FUN)(dropout_layer_2)
    else:
        dense_1 = Dense(int(192 / 2), activation=ACT_FUN)(batch_nor_2)
...
Neural_model = build_2LSTM(106, LOSS_FUNCTION, OPTIMIZER, 'relu', True)
